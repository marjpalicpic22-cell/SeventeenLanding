...
export default function ProductSubcategory() {
  ...
  const subcategoryProducts = getProductsBySubcategory(params.categoryId, params.subcategoryId);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        <section
          ...
          data-testid="subcategory-header"
        >
          <img
            src={category.image}
            alt={category.name}
            className="absolute inset-0 w-full h-full object-cover"
            data-testid="subcategory-header-image"
          />
          ...
          <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
            <div className="flex items-center gap-2 text-sm text-white/80 mb-3">
              ...
              <Link
                href={`/products/${params.categoryId}`}
                className="hover:text-white transition-colors"
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-white" data-testid="subcategory-title">
              {subcategory.name}
            </h1>
            <p className="text-white/80 mt-1" data-testid="subcategory-category-label">
              in {category.name}
            </p>
          </div>
        </section>
        ...
        {/* Products Grid */}
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12" data-testid="subcategory-products-section">
          {subcategoryProducts.length === 0 ? (
            <div className="text-center py-24" data-testid="empty-subcategory-message">
              <p className="text-muted-foreground text-lg mb-2">No listings yet in {subcategory.name}.</p>
